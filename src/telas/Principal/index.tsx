...
export default function Principal() {
  ...
  useEffect(() => {
    axios.get('http://192.168.0.103:3333/').then((response) => setTarefas(response.data));
  }, []);
  ...
  async function adicionarTarefa() {
    ...
    const response = await axios.post('http://192.168.0.103:3333/', { 
  ...
  async function concluirTarefa(id: number) {
    ...
    const response = await axios.patch(`http://192.168.0.103:3333/${id}`, {
  ...
  async function deletarTarefa(id: number) {
    await axios.delete(`http://192.168.0.103:3333/${id}`);
